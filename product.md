...
_Status as of the backend git-pull review. `[x]` done · `[~]` partial · `[ ]` todo._

### Setup & foundations
- [x] Repo + design exports in `design/` (all)
- [x] Android project scaffolds + runs; toolchain validated (A)
- [~] Backend FastAPI code exists + has `GET /health` — **NOT yet run** (needs `uv` + Python 3.14 + `.env`) (B)
- [ ] Google API key + `backend/.env` (GOOGLE_API_KEY, FLASH_MODEL, PRO_MODEL) on each machine (all)
- [~] §8 contract "frozen" — ⚠️ **DRIFTED:** app = per-store connect + `/orders` + memory; backend = session-scoped connect, no `/orders`, no memory. Reconcile. (C)

### Frontend — Android / Compose (12 screens — Profile added)
- [x] Home (connect-gated, mic hero, greet-by-name) (A)
- [x] Voice / Camera / Clarify / Comparing / Approve / Connect / OTP / Pay / Success / Orders (A)
- [x] Profile screen (name + phone; name → agent later) (A)
- [x] Nav state machine + connect-first gating + per-store `connectedStores` set (A)
- [x] 3 design-consistency fixes (§14) (A)
- [x] Real mic capture (MediaRecorder → base64) (A)
- [x] Real camera capture (CameraX live preview → base64) (A)
- [x] Retrofit `net/` layer (DTOs + ApiService + ApiClient, snake_case) (A)
- [x] Runtime CAMERA / RECORD_AUDIO permission prompts (`WithPermission`) (A)
- [x] Auto-read OTP (simulated SMS auto-fill) (A)
- [x] Reorder-suggestion chip on Home (A)
- [x] Dev backend-URL dialog (tap logo 10× → set/save URL) (A)
- [ ] Poppins font (needs TTF files dropped in `res/font/`) (A, optional)
- [ ] **Wire `net/` into ViewModel/screens — app is still 100% hardcoded mock, makes ZERO API calls** (A) ← the real gap

### Backend — Python / FastAPI (pulled from git — mostly built, NOT run)
- [x] `models.py` — Pydantic (§9.5) (B)
- [x] `gemini.py` — real Gemini multimodal (audio/image/text → structured Intent) (B)
- [x] `negotiation.py` — compare + 1 haggle round + budget guard + `steps` log (B)
- [x] `payment.py` — mock UPI charge → receipt (audit_chain) (B)
- [x] `orchestrator.py` — session lifecycle (in-memory) (B)
- [x] `merchants/` — **REAL Zepto/Swiggy MCP (OAuth 2.1+PKCE)** + mock Store A/B/Blinkit (B) [beyond plan]
- [~] `connect.py` — ⚠️ session-scoped single-store OTP (not per-store §8) (B)
- [~] `main.py` routes — ⚠️ **missing `GET /orders`**; connect routes are session-scoped (B)
- [~] `stores.py` — exists but **dead code** (superseded by `merchants/registry`) (B)
- [ ] `memory.py` (§9.6) — history + summarize + reorder-match + save — **NOT built** (B)
- [ ] `user_name` handling (greet-by-name / personalization prime) (B)
- [~] Verbose per-step logs — partial (a few prints) (B)
- [ ] Runnable: `uv` install + Python 3.14 + `.env` → `GET /health` green (B)

### Integration (≈0% — the biggest remaining chunk)
- [ ] Wire Android net layer → Repository → ViewModel (viewModelScope) → state-driven screens (A/C)
- [ ] Reconcile connect model (per-store app ↔ session backend) (C)
- [ ] Set app base URL to `10.0.2.2:5055` via the dev dialog (C)
- [ ] Full happy path end-to-end on the canonical scenario (C)
- [ ] Voice → real intent (Hinglish) · Photo → product identified (C)
- [ ] Reorder suggestion from memory (needs backend `memory.py`) (C)
- [ ] Out-of-stock fallback + budget-guard beats e2e (C)
- [ ] Order saved to history after pay (needs backend memory) (C)

### Should-ship (if core green)
